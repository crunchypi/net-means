...
func bubble(insertee *resultItem, items []resultItem, ascending bool) {
	for i := 0; i < len(items); i++ {
		// Clarification: '|| !items[i].set' specifies
		// that score can be set if the item is inactive.
...
// resItems2Indexes extract indexes from []resultItem.
...
// KNNCos finds 'k' nearest neighs using cosine similarity. It accepts a
...
// of indexes referencing nearest neighs given by 'vecs' that are closest
// to 'targetVec' (ordered by similarity, from best to worst).
func KNNCos(targetVec []float64, vecs func() ([]float64, bool), k int) []int {
...
// of indexes referencing nearest neighs given by 'vecs' that are closest
// to 'targetVec' (ordered by similarity, from best to worst).
func KNNEuc(targetVec []float64, vecs func() ([]float64, bool), k int) []int {
	return KNNBrute(KNNBruteArgs{
		TargetVec:        targetVec,
		VecPoolGenerator: vecs,
		K:                k,
