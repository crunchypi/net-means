...
type NewDataPointConfig = common.NewDataPointConfig

// Abbreviation.
var NewDataPoint = common.NewDataPoint

// Tweak how long a 'time unit' is (used for timeouts). It
// standardises sleep time for these tests.
var SLEEPUNIT = time.Millisecond * 10
...
func dp(v []float64, sleepUnits int) *DataPoint {
	cfg := common.NewDataPointConfig{Vec: v}
	if sleepUnits > 0 {
		cfg.Expire = time.Now().Add(SLEEPUNIT * time.Duration(sleepUnits))
		cfg.ExpireEnabled = true
	}
	dp := common.NewDataPoint(cfg)
	return &dp
}
...
func TestAddPayload(t *testing.T) {
	...
	if !c.AddPayload(dp(vec(2, 2, 2), 0)) {
...
func TestPayloadVecGenerator(t *testing.T) {
	c := newCentroid(vec(1, 1, 1))
	c.DataPoints = []payloadContainer{
		dp(vec(1, 1, 1), 0),
...
func TestDistributePayload(t *testing.T) {
	// Receivers.
	...
	c0.DataPoints = []payloadContainer{
	...
	receivers := []payloadReceiver{c1, c2}
	c0.DistributePayload(2, receivers)
